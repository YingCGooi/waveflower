...
register('transcyan',p=>p.color('oklch(.7 .2 240 / .1)'))
...
let popt = {flipTime:1,vertical:1,autorange:1,fold:1}
...
useWet({commentColor: '#444', glow:4, amount:3})
...
.s("vibraphone:3:.4,gm_vibraphone:5:.2,triangles,gm_glockenspiel:4:1").vel(1/3)
.pan(`<.8 .9 1>`.fast(3)).hpf(990)
.sus(1/3).dec(1/2)
.off(0, x=>x.bpf(12800))
.rel(1/7).postgain(1/3)

const BELLS = (p, lp) => note(p).fast(3).add(note(14))
.s("xylophone_hard_ff").vel(1/2)
.off(1/49, x=>x.s("gm_glockenspiel:4,gm_glockenspiel:1:.33")
...
const CHIMESDOWN = s("chimes2")
.postgain(1/6).room(2/3).size(7).hpf(770).lpf(16000)
...
const chrd = `<Am@2 F2 C2  Am@2 F C2  Am@2 G2 C  <Am F2>@2 <Em7 G2> <F2 Am F2 C>>`.transcyan()
const b1 = `<
...
arrange(
  ...
  [8, stack(CHIMESUP.mask("<1 0@31>"),
    ...
    MELODY(m1).lpfAt(2800,1400,64).early(24),
