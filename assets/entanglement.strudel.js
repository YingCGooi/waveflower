...
setcpm(120 / 4)
ENV.scaleRadius = 2; ENV.lineWidthEnd = 3; ENV.lineWidthStart = 3; ENV.blurFactor = 1; ENV.hueOffsets = 33
const opt= {height:40,width:800,thickness:4,scale:1.5}
...
.lpfTri(400,4800,32).lpe(1).lpq(0).compressor(-14).hpf(40).pg(1/4)
...
$SUPERSQUARE:stack(
  ...
  just(notes).s("square").vib(2).vibmod(1/12).add(note(12)).vel(3/4),
  just(notes).s("square").vib(4).vibmod(1/4).pan(2/3).add(note(24)).vel(5/8),  
).att(1/32).color('oklch(.7 .2 0)')
  .room(1/4).size(7).delay(1/4)
  .decay(1).sustain(2).rel(1/8)
  .lpfTri(12000,20000,32).pan("[1 .67]*4").gain("[1 .8]*4").delayfb(0.6)
...
const bi = `<
  A1@3 E1 F1@3 G1 E2@3 E1 F1@3 G1
...
$GLOSSING_FATBASS: just(bm).s("saw").hpf(270).pan(1/2)
  ...
  .postgain(2/3).glide(0.12).o(5)._scope(opt)
...
>*8`.as("note")).scale("A4:minor").add(note("0,0.33")).room(0.8).delay(0.2)
  ...
  .pan(perlin.seed(3).slow(4).range(0.2, 0.6)).pg(1/2).o(5).color('oklch(.88 .2 0)')._scope(opt)
