...
//          Even in the midst of loss and confusion, we can somehow find hope in one another
// @license CC BY-NC-SA
...
useWet({amount:.7,hueShift:-60,letterSpacing:-1,commentColor:'#FFFFFF09',gutterColor:'#FFF9',brightness:.6})
setcpm(124 / 4)
ENV.scaleRadius = 2; ENV.lineWidthEnd = 4; ENV.lineWidthStart = 4; ENV.blurFactor = .7; ENV.hueOffsets = 45
const opt= {height:40,width:640,thickness:4,scale:1.5}
...
[C1, C2, C3]@4
[F1, F2]@4
>*2`).vel(`<1@4 2.1@4 1.33@4 2@4>*2`)
.s("supersaw").o(5)
.detune(1/9).unison(2).spread(1)
.sus(4/3).rel(1/2).att(1/3).color('oklch(.7 .2 260)')
.lpfAt(2000,6400,32).lfo({s:1/12,depthabs:4000,shape:0})
.lpe(1).lpq(1/2).compressor(-12).pg(1/4)._scope(opt)

const chrd = cat(
  "[C2, C, G, C4]@3     [D2, F3, D4]",  "[A1, A2, A3, A4]@3    [G1, G2, B3]",
  "[C2, C, E4, G]@3 [D2, F, D, A]", "[A1, A2, E, C]@3  [G2, D3, B3, D3]",
  "[C2, C, G, C4]@3     [D2, D3, F4]",  "[A1, A2, C, E4]@3 [G2, D3, B3]",
  "[C2, C, E4, G]@3 [D2, F, D, A]", "[A1, A2, C, E]@3  [G1, G2, G, B2]",  
)
const interlude = cat(
  "[A2, A3, E3, A4]@3     [E2, E3, G3]",    "[F1, F2, C3, F3]@3 [G1, G2, D3]",
  "[A2, A3, E3, E4]@3 [E2, G2, E3, G3]","[F2, A3, C3, F3]@3 [G2, G3, B3]",
  "[A2, A3, C2, A4]@3     [E2, E3, G3]",    "[F1, F2, A2, F3]@3 [G2, D3, B3]",
...
.lpfTri(400,4800,32).lpe(1).lpq(0).compressor(-14).hpf(40).pg(1/6)
...
$TRIANGLE:
  just(m2).s("tri").add(note("12")).pan(0).vel(1/2)
.off(1/128, x=>x.add(note("0")).pan(2/3).color('oklch(.7 .2 0)')._scope(opt)).att(1/64).room(1/4)
...
$SUPERSQUARE:stack(
  ...
  just(notes).s("square").vib(2).vibmod(1/4).add(note(12)).vel(3/4),
  just(notes).s("square").vib(4).vibmod(1/3).pan(2/3).add(note(24)).vel(5/8),  
).att(1/32).color('oklch(.7 .2 0)').add(note(0))
  .room(1/4).size(7).delay(1/4)
  .decay(1).sustain(2).rel(1/8)
  .lpfTri(400,14400,32).pan("[1 .67]*4").gain("[1 .8]*4").delayfb(0.6)
  .postgain(2/8)._scope(opt)
const bm = `<
...
const bi = `<
  ...
  A1@3 E1 F1@3 G1 C2@3 E2 F2@3 B1  
>*4`

$GLOSSING_FATBASS: just(bm).s("saw").hpf(270).pan(1/2)
  .lpfTri(700,3200,32).diode(1).compressor(-20)//.seg(16).dur(1/27)
  .add(stack(note("0"), note("12.1").gain(0.2))).color('red')  
  .postgain(2/4).glide(0.17).o(5)._scope(opt)

$PSYKICKS: s("[bd:2:.1,bd:5]*4").set.out(`<1 1 1 1>*4`).lpf(660).pg(3/4)
  .fit().clip(1/5).hpf(60)
  .duck("5:7").duckatt(".3:.4").duckdepth(".9:.9")
  .mask("<1@7 0 1@7 0>").pg(5/8)
$RIDE: s("rd*4").pan(1/2).hpf(4800).sus(1/2).pg(3/5)

$CLAPS: s("cp").struct(`<
~ x ~ x ~ x ~ x
~ x ~ x ~ x ~ [x ~ x x] 
>*4`).off(1/22,x=>x.s("white").vel(2))
.lpfTri(2400,4000,32).mask("<1 1>/8").pan(2/3).pg(1/4)

$GLOSSING_OHH: s("[~ [oh:2,oh]]*4").crush(5).vel(rand.range(0.77, 1))
  .pan(.8)
  .hpf(3600).almostNever(ply(2)).clip(rand.seed(2).range(0.6, 1)).rib(28, 2).pg(1/2)
  .mask("<1 1 1 1>/8")
...
>*8`.as("note")).scale("A4:minor").add(note("0,0.33")).room(0.8).delay(0.2)
  ...
  .compressor(-10).hpf(660).lpf(saw.slow(8).range(1200, 3600)).lpa(0.1).lpe(2)
  .mask("<1 1@2 1 1@2>/8").pg(.7).clip(saw.slow(8).range(1, 2)).add(note(rand.mul(0.1)))
  .pan(perlin.seed(3).slow(4).range(0.2, 0.6)).pg(.7).o(5).color('oklch(.88 .2 0)')._scope(opt)
